...
//count ammo
unsigned long lastReadingTime, lastReading, debouncDelay = 50;   //keep track of when the button was pressed
void countAmmo() {
    //when the magazine is inserted AND the IR gate is broken
    int IRreading = map( analogRead(IR_RECEIVER_PIN), 0, 1023, 0, 100);
    if (IRreading != lastReading) {
        if (micros() >= (lastReadingTime + debouncDelay)) {
            if (IRreading > IR_MAP_TRIP_VAL ){
...
void changeMag() {
    ...
    if (btnArr[0].isBtnPressed(true) ) {   //when the magazine is inserted
        currentAmmo = maxAmmo;  //set current ammo to the max amount of ammo
        displayAmmo();
    } else if (btnArr[0].isBtnPressed(true) ) {   //when the magazine is removed
        currentAmmo = 0;
    }
